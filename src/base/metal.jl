...
function hamiltonian_metal(cp::ClassicalProblem{T}, sym_sector = true) where T
    ...
    d_flattened_terms = MtlArray{Int32}(flattened_terms)
    d_term_starts = MtlArray{Int32}(term_starts)
    d_values = MtlArray{T}(interaction_values)
    d_ham = Metal.zeros(Complex{T}, dim)
    ...
    @metal threads=threads groups=blocks hamiltonian_kernel!(
