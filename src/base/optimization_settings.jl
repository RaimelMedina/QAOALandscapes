...
#     setup = OptSetup(),
#     fun_calls=false
#     ) where {C, S<:NoShotSimulator, P<:AbstractProblem, H<:AbstractVector, M<:AbstractMixer, T<:Real}
...
#     if !convergence_info
#         if qaoa(params) < cost
#             throw(AssertionError("Optimization did not converged. Final gradient norm is |∇E|=$(norm(Optim.g_converged(result)))"))
...
#     params::Vector{T},
#     nshots::Int=10^5;
#     setup = OptSetup(Optim.NelderMead()),
#     fun_calls=false
#     ) where {C, S<:ShotSimulator, P<:AbstractProblem, H<:AbstractVector, M<:AbstractMixer, T<:Real}
    
#     init_energy = qaoa(params, nshots)
#     f(x) = qaoa(x, nshots)
#     result = Optim.optimize(f, params, setup.method)
...
# """
# function optimizeParametersSlice(qaoa::QAOA{C, S, P, H, M}, 
#     Γmin::Vector{T}, 
#     ig::Integer,
#     gsIndex::Vector{Int};
#     tsType = "symmetric"
...
#     distn_res_m = optimize(distn, lower, upper, x0_m, Fminbox(inner_optimizer), autodiff=:forward)
#     distn_res_p = optimize(distn, lower, upper, x0_p, Fminbox(inner_optimizer), autodiff=:forward)
...
#     ig::Integer,
#     tsType = "symmetric"
#     ) where {C<:QuantumCost, S<:NoShotSimulator, P<:AbstractProblem, H<:AbstractVector, M<:AbstractMixer, T<:Real}
