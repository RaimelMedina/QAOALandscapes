...
function curateDict!(qaoa::QAOA, dict::Dict; rounding=true, digits=5)
    ...
    energy_class_dict, _ = QAOALandscapes.getEquivalentClasses(energy_of_keys; rounding=rounding, sigdigits=digits)
...
function rollDownTSWithNode!(dict, qaoa::QAOA, node::Node, generator::IDGenerator; digits = 5, threaded=true)
    results = QAOALandscapes.rollDown(qaoa, node.value, TSInitialization(), OptimizationOptimJL.BFGS())
