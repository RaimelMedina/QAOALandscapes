...
xorsat_interactions = Dict(
    ...
    15=>[
        ...
        [4, 10, 15], [5, 10, 11], [6, 11, 12], [7, 12, 13], 
        [8, 13, 14], [9, 14, 15]
    ]
);

xorsat_dict(T::Type{<:Real}, n::Int) = Dict{Vector{Int}, T}(
    val => T(1) for (index, val) ∈ enumerate(xorsat_interactions[n])
);


function sk_problem(N::Int, σ::T) where T<:AbstractFloat
    dist = Normal(T(0), σ)
